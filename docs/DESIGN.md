# Design Document: Behavioral Tests for Self-Knowledge Claims

## 1. Purpose

This document is the method contract for **Behavioral Tests for Self-Knowledge Claims**. It specifies what is
...
One-liner: Run behavior-based probes of what models can report about their own limits and processes.

Hypothesis: Run behavior-based probes of what models can report about their own limits and processes.
...
|---|---|---|
| Config schema | `selfaware.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `selfaware.configs.loader` | Hydra compose API |
| Device | `selfaware.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `selfaware.models.registry` | Pinned revisions + hardware fit |
| Hooks | `selfaware.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `selfaware.models.generation` | Chat template path recording |
| Data | `selfaware.data` | Synthetic items, splits, manifests |
| Evaluation | `selfaware.evaluation` | Metrics, failure harness, protocols |
| Ablation | `selfaware.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `selfaware.reporting` | Aggregate → MD/TeX/figures |
| Cache | `selfaware.cache` | Versioned artifact store |
...
`src/selfaware/selfaware/` are preserved across regeneration.
